...
import { majorApi, learnPathApi, reviewApi } from "@/services/api"
import {
  ArrowLeft,
  GraduationCapIcon,
  BookOpenIcon,
...
export default function MajorDetailPage() {
  ...
  const fetchMajorDetail = async () => {
    setIsLoading(true)
    try {
      const response = await majorApi.getMajor(params.id as string)
  ...
  const fetchMajorReviews = async () => {
    setLoadingReviews(true)
    try {
      const response = await reviewApi.getMajorReviews(params.id as string)
      if (response.data.success) {
  ...
  const generateLearningPath = async () => {
    ...
    try {
      ...
      const response = await learnPathApi.generateMajorPath(major.id.toString())
      if (response.data.success) {
  ...
  const handleSendComment = async () => {
    if (!commentInput.trim() || !major) return
    ...
    return (
      <div className="min-h-screen flex flex-col">
        ...
        <main className="flex-1 container py-8 flex flex-col items-center justify-center">
          <h1 className="text-2xl font-bold mb-4">专业信息不存在</h1>
          <p className="text-muted-foreground mb-6">请返回专业库浏览其他专业</p>
          <Button onClick={() => router.push("/major-library")}>返回专业库</Button>
        </main>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <Header />
      <main className="flex-1 container py-8">
        <Button variant="ghost" onClick={() => router.push("/major-library")} className="mb-6">
          <ArrowLeft className="mr-2 h-4 w-4" />
          返回专业库
        </Button>
        ...
        <div className="grid gap-6 md:grid-cols-2">
          <div className="space-y-6">
            <Card>
              ...
              <CardContent className="space-y-4">
                <div>
                  <h3 className="font-medium mb-1">专业描述</h3>
                  <p>{major.description}</p>
                </div>
                <div>
                  <h3 className="font-medium mb-1">培养目标</h3>
                  <p>{major.trainingObjectives}</p>
                </div>
                <div>
                  <h3 className="font-medium mb-1">专业特色</h3>
                  <p>{major.professionalFeatures}</p>
                </div>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-2">
                  {major.practicalTraining.map((training, index) => (
          ...
          <div className="space-y-6">
            ...
            <Card>
              ...
              <CardContent>
                <div className="flex flex-wrap gap-2">
                  {major.recommendedFor.split("/").map((trait, index) => (
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <p>根据您的简历和该专业要求，生成一份个性化学习路径，帮助您系统性地提升相关技能。</p>
