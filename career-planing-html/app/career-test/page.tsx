...
import { careerQuestions, getRandomQuestions } from "@/data/questions"
...
export default function CareerTestPage() {
  ...
  useEffect(() => {
    if (!loading && !isAuthenticated) {
      ...
      // 获取10个随机问题
      setQuestions(getRandomQuestions(careerQuestions))
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await analysisApi.analyzeJob(answers)
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 container py-8">
        <div className="mb-8">
          <h1 className="text-3xl font-bold mb-2">职业测评</h1>
          <p className="text-muted-foreground">回答以下问题，我们将为您推荐最适合的职业方向</p>
        </div>
