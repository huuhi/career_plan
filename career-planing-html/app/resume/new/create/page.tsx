...
export default function CreateResumePage() {
  ...
  // Fix the handleSubmit function to properly structure the data
  const handleSubmit = async () => {
    try {
      ...
      const response = await resumeApi.save(formattedData)
      ...
      if (response.data.success) {
        ...
        if (response.data.data && response.data.data.id) {
          router.push(`/resume/detail/${response.data.data.id}`)
        } else {
          router.push("/resume")
        }
      } else {
        throw new Error(response.data.errorMsg || "创建失败")
      }
    } catch (error: any) {
      toast.error("创建失败", {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 container py-8">
        <div className="mb-8">
          <h1 className="text-3xl font-bold mb-2">创建简历</h1>
          <p className="text-muted-foreground">填写您的简历信息，获取专业分析</p>
        </div>

        <Card>
          <CardHeader>
            <CardTitle>简历信息</CardTitle>
            <CardDescription>填写您的个人信息和工作经历</CardDescription>
          </CardHeader>
          <CardContent>
                      ...
                      <CardContent className="space-y-4">
                        <div className="grid gap-4 md:grid-cols-2">
                          ...
                          <div className="space-y-2 md:col-span-2">
                            ...
                            <Textarea
                              value={award.summary}
                              onChange={(e) => handleArrayChange("awards", index, "summary", e.target.value)}
                              placeholder="请描述获奖原因和意义"
                              rows={3}
                            />
                          </div>
                        </div>
                        ...
                        <div className="grid gap-4 md:grid-cols-2">
                          ...
                          <div className="space-y-2 md:col-span-2">
                            ...
                            <Input
                              ...
                              placeholder="例如：https://www.pmi.org/certifications/project-management-pmp"
                            />
                          </div>
                        </div>
                      </CardContent>
